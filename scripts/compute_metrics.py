...
def compute_metrics(file_path, gold_file_path=None, zero_shot=False):
    ...
    Args:
        file_path (str): Path to model output (in TSV or jsonl format).
    ...
    df = pd.read_json(file_path, orient="records",
                      lines=True) if is_jsonl else pd.read_csv(file_path, sep="\t")

    for _, line in df.iterrows():

        gold, pred, context = line["target"], line["prediction"], line["input"]
...
def main():
    ...
    parser.add_argument("--model_output", type=str,
                        help="Path to model output in TSV or jsonl format.", required=True)
