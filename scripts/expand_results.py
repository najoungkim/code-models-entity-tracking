    ...
    with open(args.input_file, encoding="utf-8") as in_f, \
        ...
        for i, exp_line in enumerate(exp_f):
            ...
            if i % args.num_boxes == 0:
                ...
                target, pred, context = cols["target"], cols["prediction"], cols["input"]
                ...
                if "Box " in pred:
                    indiv_predictions = re.split(r', and Box|, Box| and Box|; Box', pred)
                else:
                    if "is in" in pred or "are in" in pred or "is nothing in" in pred:
                        pred = re.sub(r'in Container ([A-Z0-9]) (and )?the', "in Container \g<1>, the", pred)
                        pred = re.sub(r'(, ([a-zA-Z- ]+) (is|are) in Container ([A-Z0-9]))', ", Container \g<4> contains \g<2>", pred)
                        pred = re.sub(r'(, )?[Tt]here is nothing in Container ([A-Z0-9])', ", Container \g<2> is empty", pred)
                    indiv_predictions = re.split(r', and Container|, Container| and Container', pred)

                if indiv_predictions[0].startswith("is empty,"):
                    indiv_predictions[0] = "is empty"
                
                indiv_predictions = indiv_predictions[0:1] + sorted(indiv_predictions[1:])
                ...
                if len(indiv_predictions) < args.num_boxes:
                    ...
                    for indiv_pred in list(indiv_predictions[1:]):
                        ...
                        #box A, box B, ...
                        elif ord(box_name) >= 65 and ord(box_name) < 65 + args.num_boxes:
                ...
                assert len(indiv_predictions) == args.num_boxes, f"wrong number of predictions:\n{'||'.join(indiv_predictions)}"
                ...
                assert contexts[-1].replace("Box 0 contains .", ".") == context.replace("Box 0 contains .", "."), f"contexts do not match, likely because --input_file and --expanded_dataset are different datasets\n{contexts[-1]}\n{context}"
                # the first prediction has the format of the individual predictions
                # e.g., "contains the plane" or "is empty".
                cleaned_indiv_predictions.append(indiv_predictions[0].strip().strip("."))
                with_verb = indiv_predictions[0].strip().startswith("contains") or indiv_predictions[0].strip().startswith("is")
